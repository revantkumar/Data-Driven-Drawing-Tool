...
Gneiss.defaultGneissChartConfig = {
	container: "#svgcanvas", //css id of target chart container
	svg_place: "#canvasBackground",
...
{	
	...
	this.svgId = function Gneiss$svgId(elem) {
		if (!arguments.length) {
			return svgId;
		}
		svgId = elem;
	...
	this.build = function Gneiss$build(config) {
		...
		*/
		...
		g.svgId(config.svg_place.slice());
  ...
  panZoomInstance.zoom(1)
		...
		g.width(g.containerElement().width()-2500); //save the width in pixels
		g.height(g.containerElement().height()-2700); //save the height in pixels
	...
	this.resize = function Gneiss$resize(){
		...
		*/
		...
		g.width(g.containerElement().width()-2500);
